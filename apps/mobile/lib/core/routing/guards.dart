...
/// Route guard (Â§11: go_router guards).
///
/// Pure function of (auth state, location) â†’ redirect or null, so it is
/// unit-testable without a widget tree. The three states map to three
/// regions of the app:
///
///   unknown   â†’ hold on the splash until the session is restored
///   signedOut â†’ the auth screens only
///   signedIn  â†’ everything except the auth screens
String? authRedirect(AsyncValue<AuthState> auth, String location) {
  ...
  return switch (auth.value) {
    ...
    AuthSignedIn() => (isAuthRoute || isSplash) ? Routes.today : null,
  };
}
