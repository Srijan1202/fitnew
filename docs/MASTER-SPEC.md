...
| **Windows: project and pub cache on different drives** (e.g. project on `D:`, `%LOCALAPPDATA%\Pub\Cache` on `C:`) | Every Kotlin plugin fails `compileDebugKotlin` with `this and base files have different roots`; three-plugin `BUILD FAILED` | Kotlin incremental compilation stores paths relative to the project root and there is no relative path across drives. `kotlin.incremental=false` in `android/gradle.properties` (committed). Run `gradlew --stop` first so stale daemons pick it up. Linux CI is unaffected. Recorded 2026-09-21. |
